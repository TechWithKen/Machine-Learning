...
titanic_ship.drop(columns=["Survived","PassengerId", "Name", "Ticket", "Cabin",  "Parch"], inplace=True)
X = titanic_ship
y = survived
...
base_model = [
    ('xgb', XGBClassifier(n_estimators=300, learning_rate=0.8, random_state=42, subsample=1.0, max_depth=15)),
    ("rdf", RandomForestClassifier(n_estimators=200, criterion="gini", max_depth=15, max_features="log2", max_leaf_nodes=2, random_state=42)),
    ("svm", SVC(probability=True, C=0.01, kernel="rbf", gamma="scale")),
    ("tree", DecisionTreeClassifier(criterion='entropy', splitter="best", max_depth=20, random_state=42))
]

meta_model = LogisticRegression(C=1, class_weight="balanced", max_iter=2000)

stacked_model = StackingClassifier(estimators=base_model, final_estimator=meta_model, passthrough=True)
chi2_selector = SelectKBest(score_func=chi2, k=7)
...
titanic_test.drop(columns=["PassengerId", "Name", "Ticket", "Cabin", "Parch"], inplace=True)
...
new_titanic_data = new_titanic_data.to_csv("titaniu.csv", index=False)
...
def divide_home_away_team_results(league_result):

    # for home team H = Win, A = Loss
    # for the away team, H = Loss, A = Win
    home = create_outcome("home_team", {"H": "W", "A": "L", "D": "D"}, league_result)
    away = create_outcome("away_team", {"A": "W", "H": "L", "D": "D"}, league_result)
...
def each_team_total_goals(league_result):
    home = get_team_goals("home_team", "home_goals", league_result)
    away = get_team_goals("away_team", "away_goals", league_result)
...
def goal_conceded(league_result):

    home = count_goals_conceded("home", "away", league_result)
    away = count_goals_conceded("away", "home", league_result)
